...
const getQueue = async () => {
    try {
        const songData = await Song.find().filter({ ends: { $gt: Date.now() / 1000 } });
        return songData;
    } catch (err: any) {
        console.log(err);
        return [];
...
async function main () {
    try {
        ...
        const prog = new anchor.Program(idl,"FmgeN87iJJX4ZQi9rE7PPehA1TJr1xAQCWoHt5oKCPSz",provider);

        prog.addEventListener("SongAdded", async (ev: any, slot: any, sign: any) => {
            const title = ev.title;
            const artist = ev.artist;
            const length = GetSongLength(title, artist);
            const queue = await getQueue();
            const starts = queue[queue.length - 1]?.ends + 1 || Date.now() / 1000;
